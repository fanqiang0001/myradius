...
typedef struct module_t {
	...
	instantiate_t		instantiate;		//!< Function to use for instantiation. //配置文件解析时，初始化对应模块实例
	detach_t		detach;			//!< Function to use to free module instance.
	packetmethod		methods[MOD_COUNT];	//!< Pointers to the various section functions. //收到请求时，根据阶段调用对应的功能函数
} module_t;
